...
namespace UniSAEmloyeeEmployerCertificationAndEngagement.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        private StratisApiFullfilRequestComponent _stratisApiComponent;
        //public RoleManager<IdentityRole> RoleManagerFactory() => new RoleManager<IdentityRole>(new RoleStore<IdentityRole>());

        public AccountController(/*ApplicationUserManager userManager, ApplicationSignInManager signInManager,*/ RoleManager<IdentityRole> roleManager, UnitOfWork unitOfWork)
        {
        ...
        private async Task SaveWalletToDB(string walletName, string accountName, LoginViewModel model)
        {
            try
            {
                var accountAddresses = await _stratisApiComponent.GetAccountAddresses(walletName, accountName);

                var hasAddedAccountToDb = _unitOfWork.StratisAccountRepository.Add(new UniSA.Domain.StratisAccount
                {
                    AccountName = accountName,
                    EmailAddress = model.Email,
                    AccountWalletName = walletName,
                ...
                if (hasAddedAccountToDb)
                {
                    var hasChanges = _unitOfWork.UniSADbContext.ChangeTracker.HasChanges();
                    _unitOfWork.SaveChanges(); 
                }
                var hasLoadedWallet = await _stratisApiComponent.LoadWallet(walletName, model.Password);
            }
            catch
            {
                throw;
        ...
        public ActionResult Register()
        {
            ...
            ViewBag.StraisAccountIdList = GetStraisAccountIdList();
            var registrationModel = new RegisterViewModel();
            return View(registrationModel);
        }

        private List<SelectListItem> GetStraisAccountIdList()
        {
            if (_unitOfWork.StratisAccountRepository.GetAll().Any())
            {
                return _unitOfWork.StratisAccountRepository.GetAll().Select(r => new SelectListItem { Text = r.AccountName, Value = r.StratisAccountId.ToString() }).ToList();
            }
        ...
        private List<SelectListItem> GetAllRoles()
        {
            ...
            var roles = _roleManager.Roles.Select(r => new SelectListItem { Text = r.Name, Value = r.Name });
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    /*var role = _roleManager.Roles.FirstOrDefault(r => r.Name.ToLower().Equals(model.Roles.ToLower()));
                    var useridRole = new IdentityUserRole { RoleId = role.Id, UserId = user.Id };
                    var userInroles = user.Roles.Contains(useridRole); 
                    if (!userInroles)
                    {
                        var idUserRole = new IdentityUserRole();
                        idUserRole.UserId = user.Id;
                        idUserRole.RoleId = role.Id;

                        user.Roles.Add(idUserRole);
                    }*/
